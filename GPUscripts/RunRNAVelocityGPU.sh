...
echo "Where is the source code of VeloCD stored on your computer? NOTE: This script refers to VeloCD adapted for GPU usage, so your file path may different. Please type it in the format: /user/Xfolder/Yfolder"
...
then
    ...
    then
        ...
        python3 -Wignore $CodeLocation/RNA_Velocity_Main_GPU.py $Embed1 "$wd" "$splicedName" "$unsplicedName" "${MainExpressionFileLoc}/$metafile" "${MainExpressionFileLoc}/$Meta2Name" "$PCAFS" "Results/" "$CCFSOption" "False" "$UMAPDistNum" "$UMAPDist" "$CodeLocation/" "$Typeof1" "$GIFRUN" "$SecondMeta" "$Typeof2" "$RS" "$CI" "$CIIteration" "$tSNEYesNoGPUOnly"
    fi
