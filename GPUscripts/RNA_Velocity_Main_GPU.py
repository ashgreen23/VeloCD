...
# --- GPU-accelerated imports (RAPIDS cuML + CuPy) ---
# Falls back to CPU (sklearn/umap-learn) if GPU is not available
try:
    import cupy as cp
    ...
    GPU_AVAILABLE = True
    print("GPU acceleration enabled (RAPIDS cuML + CuPy detected)")
except ImportError:
    ...
    GPU_AVAILABLE = False
    print("GPU not available - falling back to CPU (sklearn/umap-learn)")
...
def _gpu_colDeltaCor(X, delta_X):
    """GPU-accelerated full column-delta correlation matrix using CuPy.
    Replaces colDeltaCor / colDeltaCorSqrt / colDeltaCorLog10 when GPU is available.
    X and delta_X should already have any transform (sqrt/log) applied before calling.
    Shape: (genes, cells) -> returns (cells, cells) correlation matrix.
    """
    X_gpu = cp.asarray(X, dtype=cp.float32)
    dX_gpu = cp.asarray(delta_X, dtype=cp.float32)
    # Mean-center along genes (axis=0)
    X_m = X_gpu - X_gpu.mean(axis=0, keepdims=True)
    dX_m = dX_gpu - dX_gpu.mean(axis=0, keepdims=True)
    # Correlation: (X_m.T @ dX_m) / (||X_m|| * ||dX_m||)
    numer = X_m.T @ dX_m
    norm_X = cp.linalg.norm(X_m, axis=0)
    norm_dX = cp.linalg.norm(dX_m, axis=0)
    denom = norm_X[:, None] * norm_dX[None, :]
    denom = cp.maximum(denom, 1e-10)  # avoid division by zero
    result = numer / denom
    return cp.asnumpy(result).astype(np.float64)

def _gpu_colDeltaCorPartial(X, delta_X, neigh_ixs):
    """GPU-accelerated partial column-delta correlation (only for neighbor pairs).
    Returns a dense (cells, cells) matrix with non-zero entries only at neighbor positions.
    """
    n_cells = X.shape[1]
    X_gpu = cp.asarray(X, dtype=cp.float32)
    dX_gpu = cp.asarray(delta_X, dtype=cp.float32)
    ...
    norm_X = cp.linalg.norm(X_m, axis=0)
    norm_dX = cp.linalg.norm(dX_m, axis=0)
    # Compute full correlation on GPU then mask on CPU (still faster than CPU-only partial)
    numer = X_m.T @ dX_m
    denom = norm_X[:, None] * norm_dX[None, :]
    denom = cp.maximum(denom, 1e-10)
    full_corr = cp.asnumpy(numer / denom).astype(np.float64)
    # Mask: only keep neighbor entries
    result = np.zeros((n_cells, n_cells), dtype=np.float64)
    for i in range(neigh_ixs.shape[0]):
        result[i, neigh_ixs[i]] = full_corr[i, neigh_ixs[i]]
    return result

def _gpu_transition_prob(corrcoef, sigma_corr, embedding_knn):
    """GPU-accelerated transition probability: exp(corrcoef/sigma) * knn, row-normalized."""
    cc_gpu = cp.asarray(corrcoef, dtype=cp.float32)
    knn_gpu = cp.asarray(embedding_knn, dtype=cp.float32)
    tp = cp.exp(cc_gpu / sigma_corr) * knn_gpu
    row_sums = tp.sum(axis=1, keepdims=True)
    row_sums = cp.maximum(row_sums, 1e-10)
    tp /= row_sums
    return cp.asnumpy(tp).astype(np.float64)

def _gpu_embedding_shift(transition_prob, embedding_knn, embedding):
    """GPU-accelerated delta_embedding calculation from transition prob and unitary vectors."""
    emb_gpu = cp.asarray(embedding, dtype=cp.float32)
    tp_gpu = cp.asarray(transition_prob, dtype=cp.float32)
    knn_gpu = cp.asarray(embedding_knn, dtype=cp.float32)
    ndims = embedding.shape[1]
    # unitary_vectors: shape (ndims, ncells, ncells)
    uv = emb_gpu.T[:, None, :] - emb_gpu.T[:, :, None]
    norms = cp.linalg.norm(uv, ord=2, axis=0)
    norms = cp.maximum(norms, 1e-10)
    uv /= norms
    for d in range(ndims):
        cp.fill_diagonal(uv[d], 0)
    delta = (tp_gpu * uv).sum(2)
    knn_sum = knn_gpu.sum(1)
    knn_sum = cp.maximum(knn_sum, 1e-10)
    delta -= (knn_gpu * uv).sum(2) / knn_sum[None, :]
    delta = delta.T
    return cp.asnumpy(delta).astype(np.float64), cp.asnumpy(uv).astype(np.float64)

def _gpu_pairwise_distances(X):
    """GPU-accelerated pairwise Euclidean distance matrix using CuPy."""
    X_gpu = cp.asarray(X, dtype=cp.float32)
    # ||a-b||^2 = ||a||^2 + ||b||^2 - 2*a.b
    ...
    return cp.asnumpy(cp.sqrt(dist_sq)).astype(np.float64)
...
class VelocytoLoom:
    ...
    def basicUMAP(self, num_nei, make_gify: bool=True, distance: float=0.1, metric_method: str="euclidean", typegroup: str="integers", do3D: str="True", UMAP_3D_loc: str="/location/", ChangeShape: str="True", Colours: str="Paired"):
       if ChangeShape == "True":
           ...
           corresp = ["o", "s", "^", "P", "D", "X", "*", "p", "+", ">", "<"]
           if GPU_AVAILABLE:
               trans2comp = cuUMAP(n_neighbors = num_nei, n_components=2).fit(self.S_norm.T)
           else:
               trans2comp = umap.UMAP(n_neighbors = num_nei, n_components=2).fit(self.S_norm.T)
           self.UMAPembed = np.array(trans2comp.embedding_) if GPU_AVAILABLE else trans2comp.embedding_
           if do3D == "True":
               os.chdir(UMAP_3D_loc)
               if GPU_AVAILABLE:
                   trans = cuUMAP(n_neighbors = num_nei, min_dist=distance, n_components=3, metric=metric_method).fit(self.S_norm.T)
               else:
               ...
               self.UMAPembed3d = np.array(trans.embedding_) if GPU_AVAILABLE else trans.embedding_
               count = 0
               my_types = {}
               for the_type in mark:
                   my_types[the_type] = corresp[count]
    ...
    def estimate_transition_probUMAP(self, hidim: str="Sx_sz", embed: str="UMAPembed", transform: str="sqrt",ndims: int=None, n_sight: int=None, psc: float=None,knn_random: bool=False, sampled_fraction: float=0.3,sampling_probs: Tuple[float, float]=(0.5, 0.1), max_dist_embed: float=None, n_jobs: int=4, threads: int=None, calculate_randomized: bool=True, random_seed: int=15071990, **kwargs) -> None:
      ...
      if knn_random:
          ...
          nn = NearestNeighbors(n_neighbors=n_neighbors) if GPU_AVAILABLE else NearestNeighbors(n_neighbors=n_neighbors, n_jobs=n_jobs)
          ...
          if transform == "log":
              ...
              self.corrcoef_UMAP = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
          elif transform == "logratio":
              log2hidim = np.log2(hi_dim + psc)
              delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
              self.corrcoef_UMAP = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim, neigh_ixs, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + psc) - log2hidim
                  self.corrcoef_UMAP_random = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim_rndm, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim_rndm, neigh_ixs, threads=threads)
          elif transform == "linear":
              self.corrcoef_UMAP = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  self.corrcoef_UMAP_random = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs, threads=threads)
          elif transform == "sqrt":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
          else:
          ...
          if transform == "log":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
          elif transform == "logratio":
              log2hidim = np.log2(hi_dim + psc)
              delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
              self.corrcoef_UMAP = _gpu_colDeltaCor(log2hidim, delta_hi_dim) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + 1) - log2hidim
                  self.corrcoef_UMAP_random = _gpu_colDeltaCor(log2hidim, delta_hi_dim_rndm) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim_rndm, threads=threads)
          elif transform == "linear":
              self.corrcoef_UMAP = _gpu_colDeltaCor(hi_dim, hi_dim_t - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t - hi_dim, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  self.corrcoef_UMAP_random = _gpu_colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim, threads=threads)
          elif transform == "sqrt":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
          else:
              raise NotImplementedError(f"transform={transform} is not a valid parameter")
    ...
    def calculate_embedding_shiftUMAP(self, sigma_corr: float=0.05, expression_scaling: bool=False, Myneighbours: float=None, scaling_penalty: float=1., metadataToUse: str="metadata.csv", pathToTransitionProbabilityCode: str="/", grouptype: str="Strings", secondgroup: str="metadataVersion.csv", manyfeatures: str="True", grouptype2: str="Strings", Randomsampling: str="False", extraNNvalue: int=5, DE_location: str="/location/", selfCC_folder: str="/location/", VecX_location: str="/location/", VecY_loc: str="/location/", TP_folder: str="/location/", UMAP_2D_GroupLevelTP: str="/location/", DoSecondPred: str="no", UserThreshold: float=0.5) -> None: #Use the transition probability to project the velocity direction on the embedding
        ...
        if self.corr_calc == "full" or self.corr_calc == "knn_random":
            ...
            if GPU_AVAILABLE:
                self.transition_prob_UMAP = _gpu_transition_prob(self.corrcoef_UMAP, sigma_corr, self.embedding_UMAP_knn.toarray())
            else:
            ...
            if hasattr(self, "corrcoef_random"):
                ...
                if GPU_AVAILABLE:
                    self.transition_prob_UMAP_random_UMAP = _gpu_transition_prob(self.self.corrcoef_random_UMAP, sigma_corr, self.embedding_UMAP_knn.toarray())
                else:
    ...
    def calculate_embedding_shiftUMAP3D(self, sigma_corr: float=0.05, expression_scaling: bool=False, Myneighbours: float=None, scaling_penalty: float=1., metadataToUse: str="metadata.csv", pathToTransitionProbabilityCode: str="/", grouptype: str="Strings", secondgroup: str="metadataVersion.csv", manyfeatures: str="True", grouptype2: str="Strings", Randomsampling: str="False", extraNNvalue: int=5, DE_location: str="/location/", selfCC_folder: str="/location/", VecX_location: str="/location/", VecY_loc: str="/location/", VecZ_loc: str="/location/", TP_folder: str="/location/", UMAP_3D_GroupLevelTP: str="/location/", DoSecondPred: str="no", UserThreshold: float=0.5) -> None: #Use the transition probability to project the velocity direction on the embedding
        ...
        if self.corr_calc == "full" or self.corr_calc == "knn_random": #thit way:
            ...
            # NOTE if knn_random this could be made much faster either using sparse matrix or neigh_ixs
            if GPU_AVAILABLE:
                self.transition_prob_UMAP_3d = _gpu_transition_prob(self.corrcoef_UMAP_3d, sigma_corr, self.embedding_UMAP_knn_3d.A)
            else:
            ...
            if hasattr(self, "corrcoef_random"):
                logging.debug("Calculate transition probability for negative control")
                if GPU_AVAILABLE:
                    self.transition_prob_UMAP_random_UMAP_3d = _gpu_transition_prob(self.self.corrcoef_random_UMAP_3d, sigma_corr, self.embedding_UMAP_knn_3d.A)
                else:
    ...
    def estimate_transition_probUMAP3D(self, hidim: str="Sx_sz", embed: str="UMAPembed3d", transform: str="sqrt",ndims: int=None, n_sight: int=None, psc: float=None,knn_random: bool=False, sampled_fraction: float=0.3, sampling_probs: Tuple[float, float]=(0.5, 0.1), max_dist_embed: float=None, n_jobs: int=4, threads: int=None, calculate_randomized: bool=True, random_seed: int=15071990, **kwargs) -> None:
      ...
      if knn_random:
          ...
          if transform == "log":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP_3d = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
          elif transform == "logratio":
              log2hidim = np.log2(hi_dim + psc)
              delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
              self.corrcoef_UMAP_3d = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim, neigh_ixs, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + psc) - log2hidim
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim_rndm, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim_rndm, neigh_ixs, threads=threads)
          elif transform == "linear":
              self.corrcoef_UMAP_3d = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs, threads=threads)
          elif transform == "sqrt":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP_3d = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
          else:
              raise NotImplementedError(f"transform={transform} is not a valid parameter")
          ...
          self.embedding_UMAP_3d = embedding
          logging.debug("Calculate KNN in the embedding space")
          nn = NearestNeighbors(n_neighbors=n_neighbors) if GPU_AVAILABLE else NearestNeighbors(n_neighbors=n_neighbors, n_jobs=n_jobs) #auto algorithm means it will tyr and determine the best method itself.000
          nn.fit(embedding)  # NOTE should support knn in high dimensions
          ...
          if transform == "log":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP_3d = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
          elif transform == "logratio":
              log2hidim = np.log2(hi_dim + psc)
              delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
              self.corrcoef_UMAP_3d = _gpu_colDeltaCor(log2hidim, delta_hi_dim) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + 1) - log2hidim
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCor(log2hidim, delta_hi_dim_rndm) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim_rndm, threads=threads)
          elif transform == "linear":
              self.corrcoef_UMAP_3d = _gpu_colDeltaCor(hi_dim, hi_dim_t - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t - hi_dim, threads=threads)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim, threads=threads)
          elif transform == "sqrt":
              delta_hi_dim = hi_dim_t - hi_dim
              self.corrcoef_UMAP_3d = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)
              if calculate_randomized:
                  logging.debug(f"Correlation Calculation for negative control")
                  delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                  self.corrcoef_UMAP_random_3d = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc) #30 * number of genes matrix of submatrices
          else:
              raise NotImplementedError(f"transform={transform} is not a valid parameter")
    ...
    def estimate_transition_probPCA(self, hidim: str="Sx_sz", embed: str="pcs", transform: str="sqrt",
        ...
        if knn_random:
            ...
            logging.debug("Calculate KNN in the embedding space")
            nn = NearestNeighbors(n_neighbors=n_neighbors) if GPU_AVAILABLE else NearestNeighbors(n_neighbors=n_neighbors, n_jobs=n_jobs) #+1?
            if AdjustPCs == "True":
            ...
            if transform == "log":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef_PCA = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_PCA_random = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
            elif transform == "logratio":
                log2hidim = np.log2(hi_dim + psc)
                delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
                self.corrcoef_PCA = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim, neigh_ixs, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + psc) - log2hidim
                    self.corrcoef_PCA_random = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim_rndm, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim_rndm, neigh_ixs, threads=threads)
            elif transform == "linear":
                self.corrcoef_PCA = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    self.corrcoef_PCA_random = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs, threads=threads)
            elif transform == "sqrt":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef_PCA = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_PCA_random = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
            else:
                raise NotImplementedError(f"transform={transform} is not a valid parameter")
            ...
            if transform == "log":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef_PCA = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_PCA_random = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
            elif transform == "logratio":
                log2hidim = np.log2(hi_dim + psc)
                delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
                self.corrcoef_PCA = _gpu_colDeltaCor(log2hidim, delta_hi_dim) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + 1) - log2hidim
                    self.corrcoef_PCA_random = _gpu_colDeltaCor(log2hidim, delta_hi_dim_rndm) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim_rndm, threads=threads)
            elif transform == "linear":
                self.corrcoef_PCA = _gpu_colDeltaCor(hi_dim, hi_dim_t - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t - hi_dim, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    self.corrcoef_PCA_random = _gpu_colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim, threads=threads)
            elif transform == "sqrt":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef_PCA = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)

                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_PCA_random = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
            else:
                raise NotImplementedError(f"transform={transform} is not a valid parameter")
            np.fill_diagonal(self.corrcoef_PCA, 0)
            if calculate_randomized:
    ...
    def calculate_embedding_shiftPCA(self, sigma_corr: float=0.05, expression_scaling: bool=False, scaling_penalty: float=1., metadataToUse: str="metadata.csv", 
        ...
        for nn in range(TPMin, TPMax, TPStep):  # TPMin, TPMax, TPStep should come from function arguments or parsed CLI
            ...
            # NOTE if knn_random this could be made much faster either using sparse matrix or neigh_ixs
                if GPU_AVAILABLE:
                    self.transition_prob_PCA_Red = _gpu_transition_prob(self.corrcoef_PCA, sigma_corr, self.embedding_PCA_knn.toarray())
                else:
                ...
                if hasattr(self, "corrcoef_random"):
                    logging.debug("Calculate transition probability for negative control")
                    if GPU_AVAILABLE:
                        self.transition_prob_PCA_Red_random = _gpu_transition_prob(self.corrcoef_PCA_random, sigma_corr, self.embedding_PCA_knn.toarray())
                    else:
        ...
        if ThreeDVersion == "yes":
            ...
            if self.corr_calc == "full" or self.corr_calc == "knn_random":
                ...
                # NOTE if knn_random this could be made much faster either using sparse matrix or neigh_ixs
                if GPU_AVAILABLE:
                    self.transition_prob_PCA_Three = _gpu_transition_prob(self.corrcoef_PCA, sigma_corr, self.embedding_PCA_knn.toarray())
                else:
                ...
                if hasattr(self, "corrcoef_random"):
                    logging.debug("Calculate transition probability for negative control")
                    if GPU_AVAILABLE:
                        self.transition_prob_PCA_Three_random = _gpu_transition_prob(self.corrcoef_PCA_random, sigma_corr, self.embedding_PCA_knn.toarray())
                    else:
    ...
    def perform_TSNE(self, n_dims: int=2, perplexity: float=30, initial_pos: np.ndarray=None, #not used
        ...
        if initial_pos is None:
            initial_pos = "random"
        if GPU_AVAILABLE:
            bh_tsne = TSNE(n_components=n_dims, perplexity=perplexity, n_iter=max_iter)
            self.ts = np.array(bh_tsne.fit_transform(self.pcs[:, :n_pca_dim]))
        else:
    ...
    def estimate_transition_prob(self, hidim: str="Sx_sz", embed: str="ts", transform: str="sqrt", #uses the principle components from the pca to calculate embedding coordinates for each point (embedding)
        ...
        if knn_random:
            ...
            if transform == "log":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_random = _gpu_colDeltaCorPartial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorLog10partial(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
            elif transform == "logratio":
                log2hidim = np.log2(hi_dim + psc)
                delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
                self.corrcoef = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim, neigh_ixs, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + psc) - log2hidim
                    self.corrcoef_random = _gpu_colDeltaCorPartial(log2hidim, delta_hi_dim_rndm, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(log2hidim, delta_hi_dim_rndm, neigh_ixs, threads=threads)
            elif transform == "linear":
                self.corrcoef = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t - hi_dim, neigh_ixs, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    self.corrcoef_random = _gpu_colDeltaCorPartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs) if GPU_AVAILABLE else colDeltaCorpartial(hi_dim, hi_dim_t_rndm - hi_dim, neigh_ixs, threads=threads)
            elif transform == "sqrt":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), neigh_ixs, threads=threads, psc=psc)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_random = _gpu_colDeltaCorPartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs) if GPU_AVAILABLE else colDeltaCorSqrtpartial(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), neigh_ixs, threads=threads, psc=psc)
            else:
                raise NotImplementedError(f"transform={transform} is not a valid parameter")
            ...
            if transform == "log":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_random = _gpu_colDeltaCor(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorLog10(hi_dim, np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
            elif transform == "logratio":
                log2hidim = np.log2(hi_dim + psc)
                delta_hi_dim = np.log2(np.abs(hi_dim_t) + psc) - log2hidim
                self.corrcoef = _gpu_colDeltaCor(log2hidim, delta_hi_dim) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = np.log2(np.abs(hi_dim_t_rndm) + 1) - log2hidim
                    self.corrcoef_random = _gpu_colDeltaCor(log2hidim, delta_hi_dim_rndm) if GPU_AVAILABLE else colDeltaCor(log2hidim, delta_hi_dim_rndm, threads=threads)
            elif transform == "linear":
                self.corrcoef = _gpu_colDeltaCor(hi_dim, hi_dim_t - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t - hi_dim, threads=threads)
                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    self.corrcoef_random = _gpu_colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim) if GPU_AVAILABLE else colDeltaCor(hi_dim, hi_dim_t_rndm - hi_dim, threads=threads)
            elif transform == "sqrt":
                delta_hi_dim = hi_dim_t - hi_dim
                self.corrcoef = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim) + psc) * np.sign(delta_hi_dim), threads=threads, psc=psc)

                if calculate_randomized:
                    logging.debug(f"Correlation Calculation for negative control")
                    delta_hi_dim_rndm = hi_dim_t_rndm - hi_dim
                    self.corrcoef_random = _gpu_colDeltaCor(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm)) if GPU_AVAILABLE else colDeltaCorSqrt(hi_dim, np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm), threads=threads, psc=psc)
            else:
                raise NotImplementedError(f"transform={transform} is not a valid parameter")
            np.fill_diagonal(self.corrcoef, 0)
            if calculate_randomized:
    ...
    def calculate_embedding_shift(self, sigma_corr: float=0.05, expression_scaling: bool=False, Myneighbours: float=None, scaling_penalty: float=1., metadataToUse: str="metadata.csv", pathToTransitionProbabilityCode: str="/", grouptype: str="Strings", secondgroup: str="metadataVersion.csv", manyfeatures: str="True", grouptype2: str="Strings", Randomsampling: str="False", extraNNvalue: int=5, DE_location: str="/location/", selfCC_folder: str="/location/", VecX_location: str="/location/", VecY_loc: str="/location/", TP_folder: str="/location/", tSNE_GroupLevelTP: str="/location/", DoSecondPred: str="no", UserThreshold: float=0.5) -> None: #Use the transition probability to project the velocity direction on the embedding
        ...
        if self.corr_calc == "full" or self.corr_calc == "knn_random":
            ...
            # NOTE if knn_random this could be made much faster either using sparse matrix or neigh_ixs
            if GPU_AVAILABLE:
                self.transition_prob = _gpu_transition_prob(self.corrcoef, sigma_corr, self.embedding_knn)
            else:
            ...
            if hasattr(self, "corrcoef_random"):
                logging.debug("Calculate transition probability for negative control")
                if GPU_AVAILABLE:
                    self.transition_prob_random = _gpu_transition_prob(self.corrcoef_random, sigma_corr, self.embedding_knn)
                else:
    ...
    def prepare_markov(self, sigma_D: np.ndarray, sigma_W: np.ndarray, direction: str="forward", cells_ixs: np.ndarray=None) -> None:
        ...
        if GPU_AVAILABLE:
            dist_matrix = _gpu_pairwise_distances(self.embedding[cells_ixs, :])
        else:
...
if str(sys.argv[21]) == "True":
    ...
    if GPU_AVAILABLE:
        ...
        vlm.ts = np.array(bh_tsne.fit_transform(vlm.pcs[:, :int(sys.argv[24])])) #GPU-accelerated t-SNE
    else:
        bh_tsne = TSNE(perplexity=number_neighbous)
